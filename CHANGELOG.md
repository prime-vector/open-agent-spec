...
### Fixed
- **Exponential `depends_on` cycle check on lattice DAGs (both runtimes)** — cycle detection used a recursion-stack-only DFS, which re-walks a shared subgraph once per path reaching it. On a lattice of stacked diamonds that is 2^N walks (~97 s at 28 levels, days at 40) before the first model call. Both the Python and npm runners now also track fully-explored nodes (three-colour DFS), making the check linear while preserving the diamond-DAG and true-cycle semantics. New conformance case `depends-on/deep-lattice` (a 2^40-path lattice) holds both runtimes to it. Execution semantics are unchanged: only direct dependencies run.
