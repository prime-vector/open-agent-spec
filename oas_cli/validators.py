...
def _validate_tools(spec_data: dict) -> None:
    ...
    for i, tool in enumerate(tools):
        if not isinstance(tool, dict):
            ...
            raise ValueError(
                f"tools[{i}] must be a dictionary (object), got {actual_type}. "
                f"Each tool should have 'id', 'type', and 'description' fields."
            )
        ...
        if not isinstance(tool.get("id"), str):
            ...
            raise ValueError(
                ...
                f"Provide a unique identifier, e.g., id: \"web_search\""
            )

        if not isinstance(tool.get("description"), str):
            actual_type = type(tool.get("description")).__name__ if "description" in tool else "missing"
            raise ValueError(
                f"tools[{i}].description must be a string, got {actual_type}. "
                f"Provide a description of what this tool does."
            )

        if not isinstance(tool.get("type"), str):
            actual_type = type(tool.get("type")).__name__ if "type" in tool else "missing"
            raise ValueError(
                f"tools[{i}].type must be a string, got {actual_type}. "
                f"Common values: \"function\", \"api\", \"file_operation\""
            )
...
def _validate_tasks(spec_data: dict) -> None:
    ...
    if not isinstance(tasks, dict):
        ...
        raise ValueError(
            f"Field 'tasks' must be a dictionary (object), got {actual_type}. "
            f"Example:\n  tasks:\n    task1:\n      input: {{}}\n      output: {{}}"
        )
    ...
    for task_name, task_def in tasks.items():
        if not isinstance(task_def, dict):
            ...
            raise ValueError(
                f"tasks.{task_name} must be a dictionary (object), got {actual_type}. "
                f"Each task should have 'input' and 'output' definitions."
            )
        ...
        if "tool" in task_def:
            ...
            if tool_id not in tool_ids:
                available = ", ".join(f"'{tid}'" for tid in tool_ids) if tool_ids else "none"
                raise ValueError(
                    f"tasks.{task_name} references non-existent tool '{tool_id}'. "
                    f"Available tools: {available}. Check your 'tools' section."
                )
        ...
        if not is_multi_step:
            if not isinstance(task_def.get("input"), dict):
                actual_type = type(task_def.get("input")).__name__ if "input" in task_def else "missing"
                raise ValueError(
                    f"tasks.{task_name}.input must be a dictionary (object), got {actual_type}. "
                    f"Define input schema, e.g., input: {{query: {{type: string}}}}"
                )
            if not isinstance(task_def.get("output"), dict):
                actual_type = type(task_def.get("output")).__name__ if "output" in task_def else "missing"
                raise ValueError(
                    f"tasks.{task_name}.output must be a dictionary (object), got {actual_type}. "
                    f"Define output schema, e.g., output: {{result: {{type: string}}}}"
                )
        else:
            # For multi-step tasks, validate that steps are defined
            if not isinstance(task_def.get("steps"), list):
                actual_type = type(task_def.get("steps")).__name__ if "steps" in task_def else "missing"
                raise ValueError(
                    f"Multi-step tasks.{task_name}.steps must be a list (array), got {actual_type}. "
                    f"Example:\n  steps:\n    - task: step1\n    - task: step2"
                )
