...
| 0.2 | Install the build under test | e.g. `pip install open-agent-spec==1.2.3` or `pip install .` from repo |
| 0.3 | `oa --version` | Prints **1.2.3** (or `python -m oas_cli --version` if `oa` not on PATH) |
...
|---|---------|----------------|
| 1.1 | `oa --version` | Exits 0; shows version **1.2.3** |
...
- **§1** help/version all succeed; version reads **1.2.3**.
