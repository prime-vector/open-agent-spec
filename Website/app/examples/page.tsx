  ...
  const handleGenerate = useCallback(async () => {
    ...
    if (targetLang === "python") {
      ...
      try {
        ...
        setGenerateError(
          e instanceof Error ? e.message : "Generate request failed"
        );
      } finally {
      ...
      <header className="border-border flex shrink-0 items-center justify-between border-b bg-surface-muted px-4 py-3">
        <div>
          <h1 className="text-lg font-semibold tracking-tight">
            Open Agent Spec Examples
          </h1>
          <p className="text-xs text-[var(--text-muted)]">
            Edit specs, generate agents, and run them directly from YAML.
          </p>
        </div>
            ...
            <div className="flex h-full flex-col">
              <div className="border-b border-[var(--border)] bg-surface-muted px-3 py-2 text-xs text-[var(--text-muted)]">
                ...
                <span className="font-semibold">
                  run the first task directly from this spec
                </span>
                .
              </div>
              ...
              {activeTab === "code" && (
                <GeneratedCodeTab
                  ...
                  emptyMessage={
                    generateLoading
                      ? "Generating…"
                      : "Generate agent to see scaffold."
                  }
                />
              )}
              {activeTab === "logs" && (
